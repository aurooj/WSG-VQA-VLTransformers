...
from src.pretrain.lxmert_data import InputExample, LXMERTDataset, LXMERTTorchDataset, LXMERTEvaluator
from src.lxrt.entry_spatial import set_visual_config
from src.lxrt.tokenization import BertTokenizer
from src.lxrt.modeling_spatial import LXRTPretraining

DataTuple = collections.namedtuple("DataTuple", 'dataset torchdset loader evaluator')

print(args.task_mask_lm, args.task_obj_predict, args.task_matched, args.task_qa)
...
class LXMERT:
    ...
    def forward(self, examples):
        ...
        pos = torch.from_numpy(np.stack([torch.ones(h * h) for f in train_features])).cuda()
    ...
    def evaluate_epoch(self, eval_tuple: DataTuple, iters: int=-1):
        ...
        losses_str = "The losses are "
        for name, loss in zip(LOSSES_NAME, total_losses.squeeze() / len(eval_ld)):
...
if __name__ == "__main__":
    print("print training objectives...")
    print(args.task_mask_lm, args.task_obj_predict, args.task_matched, args.task_qa)
