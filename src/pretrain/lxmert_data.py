...
Split2ImgFeatPath = {
    'mscoco_train': 'data/mscoco_imgfeat/train_features.hdf5',
    'mscoco_minival': 'data/mscoco_imgfeat/valid_features.hdf5',
    'vgnococo': 'data/gqa/gqa_spatial.h5',
}

Split2ImgPatchesPath = {
    'mscoco_train': 'data/mscoco_imgfeat/train_patches_32x32.hdf5',
    'mscoco_minival': 'data/mscoco_imgfeat/valid_patches_32x32.hdf5',
    'vgnococo': 'data/gqa/train_patches_32x32.hdf5',
}
...
class LXMERTTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        ...
        for source in self.raw_dataset.sources:
            if source == 'vgnococo':
                img_data.extend(load_spatial_gqa(Split2ImgFeatPath[source], topk))
            else:
                img_data.extend(load_spatial_data(Split2ImgFeatPath[source], topk))
        ...
        for datum in used_data:
            ...
            for sents_cat, sents in sentf.items():
                if sents_cat not in args.exclude_set:
...
class LXMERTPatchesTorchDataset(Dataset):
    def __init__(self, dataset: LXMERTDataset, topk=-1):
        super().__init__()
        self.raw_dataset = dataset
        self.task_matched = args.task_matched
        ...
        for source in self.raw_dataset.sources:
            if source == 'vgnococo':
                img_data.extend(load_patches(Split2ImgPatchesPath[source], dataset='gqa', topk=topk))
            else:
                img_data.extend(load_patches(Split2ImgPatchesPath[source], dataset='MSCOCO', topk=topk))
