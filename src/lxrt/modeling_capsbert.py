...
from .capsules_new import PrimaryCaps, ConvCaps
...
class VisualConfig(object):
    ...
    def __init__(self,
        ...
        self.max_spatial_pos_emb = hw * hw
...
#modified to return attention scores along with output features
...
class BertLayer(nn.Module):
    ...
    #modified to return self attention scores
...
class LXRTXLayer(nn.Module):
    ...
    def cross_att(self, lang_input, lang_attention_mask, visn_input, visn_attention_mask):
        # Cross Attention
        lang_att_output, attention_probs_xl = self.visual_attention(lang_input, visn_input,
                                                                    ctx_att_mask=visn_attention_mask)
        visn_att_output, attention_probs_xv = self.visual_attention(visn_input, lang_input,
...
######## Implemented by: AU,  cross attentional layers #######
...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config, norm_inputs=False, patches=False, no_caps=False):
        ...
        if self.patches:
            ...
            self.conv = nn.Sequential(nn.ZeroPad2d(1),
                                      nn.Conv2d(2048, config.hidden_size, kernel_size=3),
                                      GeLU(),
                                      nn.ZeroPad2d(1),
                                      nn.Conv2d(config.hidden_size, config.hidden_size, kernel_size=3),
                                      GeLU())
        if self.no_caps:
            ...
            if self.is_attn_routing:
                ...
                self.primary_caps = PrimaryCaps(config.hidden_size, num_prim_caps, (1, 1), P, stride=1)
                self.conv_caps = ConvCaps(num_prim_caps, num_vis_caps, (1, 1), P, stride=(1, 1), iters=3)
            self.cls_token = nn.Parameter(torch.zeros(1, 1, self.caps_dim))
        # todo: load cls from vit model, if vit init is selected
        ...
        self.pos_layer_norm = BertLayerNorm(self.caps_dim, eps=1e-12)
    ...
    def forward(self, visn_input):
        ...
        if self.no_caps:
            ...
            img_prim_caps = self.primary_caps(img)

            vis_caps, a_out = self.conv_caps(img_prim_caps)  # shape would be B, H, W, C, D

            B, H, W, C, D = vis_caps.size()

            x = torch.cat((cls_tokens, vis_caps.view(B, H * W, C * D)), dim=1)
...
class LXRTCapsulesEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        # not masking cls_token for visn_feats, hence, [:,1:,:,:]
        masked_vis_feats = torch.cat(
            [visn_feats[:, 0, :, :].unsqueeze(1), visn_feats[:, 1:, :, :] * mask[:, None, :, None]], dim=1)
        ...
        if self.shared_weights:
            # Run relational layers
            masked_vis_feats, all_attention_mask_v, masks = self.get_masked_vis_feats(self.layer, B,
                                                                                      all_attention_mask_v, seq_len, k,
            ...
            # Run relational layers
            masked_vis_feats, all_attention_mask_v, masks = self.get_masked_vis_feats(self.r_layers, B,
                                                                                      all_attention_mask_v,
                                                                                      seq_len, k, lang_sent_enc,
                                                                                      masked_vis_feats,
                                                                                      output_all_attention_masks,
                                                                                      visn_attention_mask, visn_feats,
                                                                                      self.skip_connection, masks)
...
class LXRTNoMaskCapsulesEncoder(nn.Module):
    def __init__(self, config,
                 shared_weights=False,
                 cross_attn=False,
                 cross_attn_type='cross',
    ...
    def get_masked_vis_feats(self, visn_layers, B, all_attention_mask_v, hw, k, lang_sent_enc, vis_feats,
                             output_all_attention_masks, visn_attention_mask, visn_feats, skip_connection, masks):
        B, seq_len, c, d = visn_feats.shape
        for layer_module in visn_layers:
            if self.vit_init:
                vis_feats, image_attention_probs = layer_module(vis_feats)
            else:
                vis_feats, image_attention_probs = layer_module(vis_feats, visn_attention_mask)

            # skip connection
            if skip_connection:
                # get language mask from intermediate lang features

                vis_intermediate = self.vis2langFF(visn_feats.view(B, seq_len, -1))

                # skip connection from masked capsules to intermediate visn features
                vis_feats = vis_intermediate + vis_feats

            if output_all_attention_masks:
                all_attention_mask_v.append(image_attention_probs)

        return vis_feats, all_attention_mask_v, masks
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        vis_feats = self.dropout(self.vis2langFF(visn_feats.view(B, seq_len, -1)))
        ...
        if self.shared_weights:
            # Run relational layers
            vis_feats, all_attention_mask_v, masks = self.get_masked_vis_feats(self.layer, B, all_attention_mask_v,
                                                                               seq_len, k,
                                                                               lang_sent_enc, vis_feats,
                                                                               output_all_attention_masks,
                                                                               visn_attention_mask, visn_feats,
                                                                               self.skip_connection, masks)
        else:
            # Run relational layers
            vis_feats, all_attention_mask_v, masks = self.get_masked_vis_feats(self.r_layers, B, all_attention_mask_v,
                                                                               seq_len, k, lang_sent_enc, vis_feats,
                                                                               output_all_attention_masks,
                                                                               visn_attention_mask, visn_feats,
                                                                               self.skip_connection, masks)
        # Run cross-modality layers
        if self.cross_attn:
            for i, layer_module in enumerate(self.x_layers):
                lang_feats, masked_vis_feats, attn_probs = layer_module(lang_feats, lang_attention_mask,
                                                                        vis_feats, visn_attention_mask, i)
                if output_all_attention_masks:
                    all_attention_mask_xl.append(attn_probs['attn_prob_xl'])
                    all_attention_mask_xv.append(attn_probs['attn_prob_xv'])
                    all_attention_mask_vl.append(attn_probs['attn_prob_vl'])
        ...
        return lang_feats, vis_feats, (all_attention_mask_t,
                                       all_attention_mask_v,
                                       all_attention_mask_xl,
                                       all_attention_mask_xv,
                                       all_attention_mask_vl,
