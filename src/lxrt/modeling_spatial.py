...
class VisualConfig(object):
    ...
    def __init__(self,
        ...
        #capsules param
        # self.num_prim_caps = 32 # todo: add this param to config
        # self.num_vis_caps = 32 # todo: add this param to config
        # self.pose_matrix_dim = 4 # todo: add this param to config
        # self.caps_dim = self.num_vis_caps * (self.pose_matrix_dim*self.pose_matrix_dim+1)

        self.max_spatial_pos_emb = 14*14
...
class VisualFeatEncoder(nn.Module):
    def __init__(self, config):
        ...
        # num_prim_caps = VISUAL_CONFIG.num_prim_caps #todo: add this param to config
        # num_vis_caps = VISUAL_CONFIG.num_vis_caps  # todo: add this param to config
        # P = VISUAL_CONFIG.pose_matrix_dim #todo: add this param to config

        # Object feature encoding
        self.visn_fc = nn.Linear(feat_dim, config.hidden_size)
        self.visn_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)

        self.conv = nn.Sequential(nn.Conv2d(2048, config.hidden_size, 3, padding=1),
                                  nn.ELU(),
                                  nn.Conv2d(config.hidden_size, config.hidden_size, 3, padding=1),
                                  nn.ELU())
        ...
        self.position_embeddings = nn.Embedding(VISUAL_CONFIG.max_spatial_pos_emb, config.hidden_size, padding_idx=0)
    ...
    def forward(self, visn_input):
        ...
        #
        # vis_caps = self.conv_caps(img_prim_caps) # shape would be B, H, W, C, D
        # B, H, W, C, D = vis_caps.size()
        # vis_caps = vis_caps.view(B, -1, C, D)
        # vis_caps = vis_caps.permute(0, 2, 3, 1) #B, D, H, W --> B, H, W, C, D


        device = 'cuda:'+str(img.get_device()) if torch.cuda.is_available() else 'cpu'

        position_ids = torch.arange(H*W, dtype=torch.long, device=device)
        ...
        # output = (x + y) / 2
        output = img.permute(0,2,3,1).view(B, H*W, D) + position_embeddings #todo: add position embedding after masking capsules
...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        #todo: match mask dimensions with visual feats dimensions
        B, hw, cd = visn_feats.shape
        # masked_vis_feats = visn_feats.view(B, hw, VISUAL_CONFIG.num_vis_caps, -1) * mask[:, None, :, None]

        # visn_feats = self.vis2langFF(masked_vis_feats.view(B, hw, -1))
...
class LXRTCapsulesEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.vis2langFF = nn.Linear(VISUAL_CONFIG.visual_feat_dim, config.hidden_size)
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        # masked_vis_feats = self.vis2langFF(masked_vis_feats.view(B, hw, -1))
        ...
        # Run relational layers
        for layer_module in self.r_layers:
            masked_vis_feats, image_attention_probs = layer_module(visn_feats, visn_attention_mask)
...
class LXRTModel(BertPreTrainedModel):
    ...
    def __init__(self, config, shared_weights=False):
...
class LXRTPretraining(BertPreTrainedModel):
    def __init__(self,
        ...
        # LXRT backbone
        if skip_connection:
            self.bert = LXRTCapsuleModel(config, shared_weights=shared_weights)
        else:
            self.bert = LXRTModel(config, shared_weights=shared_weights)
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, mode='lxr', skip_connection=False, shared_weights=False):
        """
