# Thanks to Kevin Duarte who provided us this implementation of CapsulesNet
import math
...
class ConvCaps(nn.Module):
    ...
    def __init__(self, B, C, K, P, stride, iters=3,
        ...
        if torch.cuda.is_available():
            self.register_buffer('ln_2pi', torch.cuda.FloatTensor(1).fill_(math.log(2 * math.pi)))
        else:
    ...
    def forward(self, x):
        ...
        if not self.w_shared:
            ...
            p_out = p_out.view(b, oh, ow, self.C,
                               self.psize)  # changed code from=> p_out.view(b, oh, ow, self.C *self.psize)
            a_out = a_out.view(b, oh, ow, self.C, 1)  # changed code from => a_out.view(b, oh, ow, self.C)
            # print('conv cap activations',a_out[0].sum().item(),a_out[0].size())
            out = torch.cat([p_out, a_out], dim=-1)  # changed code from => torch.cat([p_out, a_out], dim=3)
        else:
...
class CapsNet(nn.Module):
    def __init__(self, P=4):
        ...
        # pretrained_weights = torch.load('/home/bruce/projects/rgb_charades.pt')
        ...
        self.primary_caps = PrimaryCaps(832, 8, 9, P, stride=1)
        ...
        self.upsample3 = nn.ConvTranspose3d(128, 64, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=1,
    ...
    def forward(self, img, sent, classification):
        '''
        INPUTS:
        ...
        x = self.smooth(x)
        out_1 = x.view(-1, 4, 224, 224)
        out_2 = out_1
        out_4 = out_1
        out_8 = out_1
        out = [out_1, out_2, out_4, out_8]

        return out  # , actor_prediction
...
if __name__ == '__main__':
    ...
    print(model)
    model = model.cuda()
    fstack = torch.rand(1, 3, 4, 224, 224).cuda()
    actor = torch.Tensor([1]).view(1, 1).cuda()
    sentence = torch.rand(1, 300, 16).cuda()
    out = model(fstack, sentence, actor)
