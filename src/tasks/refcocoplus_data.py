...
class RefCOCOplusDataset:
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(json.load(open("/data/Grounded-RL2021/lxmert/data/refcoco+/annotations_%s.json" % split)))
...
class RefCOCOplusBufferLoader():
    ...
    def load_data(self, name, number):
        ...
        #     path = "data/refcoco/refcoco_testdev_spatial.h5"
        path = "/data/Grounded-RL2021/lxmert/data/refcoco+/{}_features.hdf5".format(name)
...
gqa_buffer_loader = RefCOCOplusBufferLoader()
...
"""
class RefCOCOplusTorchDataset(Dataset):
    def __init__(self, dataset: RefCOCOplusDataset):
        super().__init__()
    ...
    def __getitem__(self, item: int):
        ...
        boxes = np.ones(feats.shape[1]*feats.shape[2], dtype=np.float32) #assuming feats of shape [d, h, w]
        ...
        # else:
        return sent_id, feats, boxes, sent, target_box


class RefCOCOplusEvaluator:
    def __init__(self, dataset: RefCOCOplusDataset):
