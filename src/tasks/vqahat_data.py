...
class GQADataset:
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(json.load(open("../../data/VQA_HAT/%s.json" % split)))
        ...
        # Answers
        self.ans2label = json.load(open("../../data/VQA_HAT/trainval_ans2label.json"))
        self.label2ans = json.load(open("../../data/VQA_HAT/trainval_label2ans.json"))
...
class GQABufferLoader():
    ...
    def load_data(self, name, number):
        if name == 'testdev':
            ...
            path = "../../data/mscoco_imgfeat/valid_features.hdf5"
        elif name == 'valid':
            # path = "data/vg_gqa_imgfeat/vg_gqa_obj36.tsv"
            path = "../../data/mscoco_imgfeat/valid_features.hdf5"
        else:
            path = "../../data/mscoco_imgfeat/train_features.hdf5"
...
class GQATorchDataset(Dataset):
    def __init__(self, dataset: GQADataset):
        ...
        for datum in self.raw_dataset.data:
            datum_img_id = "COCO_val2014_{0:012d}".format(datum['image_id'])
            if datum_img_id in self.imgid2img:
                datum['image_id'] = datum_img_id
