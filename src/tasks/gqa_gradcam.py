...
class GQA:
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        self.model.eval()
        if args.gradcam:
            self.model.lxrt_encoder.model.bert.encoder.x_layers[args.block_num].visual_attention.att.save_attention = True
        # self.model.text_encoder.base_model.base_model.encoder.layer[args.block_num].crossattention.self.save_attention = True
        ...
        for i, datum_tuple in enumerate(loader):
            ...
            logit, attn_probs  = self.model(feats, boxes, sent)
            score, label = logit.max(1)
            one_hot = torch.zeros((1, logit.size()[-1]))
            one_hot[0, label] = 1
            one_hot_vector = one_hot
            one_hot = one_hot.requires_grad_(True)
            loss = torch.sum(one_hot.cuda() * logit)
            print(loss)
            ...
            if args.gradcam:
                with torch.no_grad():
                    blk=self.model.lxrt_encoder.model.bert.encoder.x_layers[-1]
                    grad_t_i = blk.visual_attention.att.get_attn_gradients().detach()
                    cam_t_i = blk.visual_attention.att.get_attn().detach()
                    gradcam = self.gradcam(cam_t_i, grad_t_i)

                    print(f"gradient shape:{grad_t_i.shape}")
                    print(f"cam shape:{cam_t_i.shape}")
                    ...
                    for qid in ques_id:
                        gradcams.append(
                            {
                                "questionId": qid,
                                "attention": gradcam[0, :, 0, :, :].cpu().detach().tolist() #only works for bsz=1
                                # gradcam shape is B,H,T,24,24; 0 for [CLS]
                            }
                        )
                        #blk = self.model.lxmert.encoder.x_layers[-1]
                # if do_analyze_model:
        ...
        evaluator.save_json(gradcam, 'snap/gqa/{output}/gradcam.json'.format(output=exp_name))
        evaluator.save_json(results, 'snap/gqa/{output}/val_attentions_cross_2.json'.format(output=exp_name))
        evaluator.save_json(activations, 'snap/gqa/{output}/activations.json'.format(output=exp_name))
